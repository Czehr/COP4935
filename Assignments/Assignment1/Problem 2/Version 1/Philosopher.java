...
public class Philosopher implements Runnable {
    ...
    private void eat() {
        ...
        // Attempt to pick up the left chopstick if we don't have it already
        if (left == -1) left = pickupChopstick(LEFT);

        // Attempt to pick up the right chopstick if we don't have it already
        if (right == -1) right = pickupChopstick(RIGHT);
